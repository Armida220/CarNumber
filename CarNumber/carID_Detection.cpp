...
// 		for (int i = 0 ; i < 4 ; ++i)
// 		{
// 			cv::line(img_contours, vertices[i], vertices[(i + 1) % 4], cv::Scalar(255, 0, 0), 1);
// 		}
	...
	for (int i = 0 ; i < char_rects.size() ; ++i)
	{
		...
		char_rects[i].points(vertices);
		for (int j = 0 ; j < 4 ; ++j)
		{
			cv::line(img_contours, vertices[j], vertices[(j + 1) % 4], cv::Scalar(255, 0, 0), 1);
		}
	}
	//cv::imshow("img_countours", img_contours);
	//cv::waitKey(0);
...
void char_sort(vector<RotatedRect > & in_char)
{
	...
	{
		mp.insert(std::pair<int, float>(i, in_char[i].center.x));
	}

	std::vector<std::pair<int, float> > vec(mp.begin(), mp.end());
	std::sort(vec.begin(), vec.end(), cmp_by_value);

	std::vector<RotatedRect> copy_in_char;
	copy_in_char.resize(in_char.size());
	for (int i = 0 ; i < in_char.size() ; ++i)
	{
		copy_in_char[i] = in_char[vec[i].first];
	}

	std::copy(copy_in_char.begin(), copy_in_char.end(), in_char.begin());
}
